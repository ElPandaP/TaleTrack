...
function ActivityItem({ event, now }: { event: TrackingEvent; now: number }) {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-border last:border-0">
      ...
      <span className="text-xs text-muted-foreground/60 shrink-0">{timeStr}</span>
    </div>
  );
}
...
export default function DashboardClient({ initialEvents, initialBooks, initialNow }: Props) {
  const { user } = useAuth();
  ...
  const latestPerMedia = Object.values(
    initialEvents.reduce<Record<number, TrackingEvent>>((acc, ev) => {
  ...
  const recentActivity = [...initialEvents]
    .sort((a, b) => new Date(b.eventDate).getTime() - new Date(a.eventDate).getTime())
    .slice(0, 12);
          ...
          {initialBooks.length > 0 && (
            <div className="mt-8">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {initialBooks.slice(0, 12).map((book) => (
                  <div key={book.id} className="shrink-0 w-20 group cursor-pointer">
                    {book.coverUrl ? (
                      <Image
                        src={book.coverUrl}
                        alt={book.title}
